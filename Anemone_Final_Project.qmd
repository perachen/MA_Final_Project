...
This project focus on an analysis of *Anemone coronaria* data that was collected by *Norah Saabnah* from the *School of Plant Sciences and Food Security.*
...
To apply Beta Regression I had to change some observations from 0 to 0.001 in the two response variables - the rate of the petal damage and the leaf damage:
...
The graph and the regression analysis show no evidence of the color's effect on the herbivory rate for every reasonable significance level. As can be seen in the appendix, the results from the [Linear Regression](#linear-regression-1) and the [Categorical Regression](#categorical-regression-1) are consistent with the results of the beta regression (that is presented below).

![](herbivory_by_color_plot.png){fig-align="center" width="528" height="253"}
...
According to the regression analysis, there is no evidence for the effect of the color over the florivory rate for every reasonable significance level. As can be seen in the appendix, the results from the [Linear Regression](#linear-regression-1) and the [Categorical Regression](#categorical-regression-1) are consistent with the results of the beta regression (that is presented below).

![](florivory_by_color_plot.png){fig-align="center" width="528" height="253"}
...
![](herbivory_by_weight_of_seeds.png){fig-align="center" width="526" height="335"}
...
![](florivory_by_number_of_seeds.png){fig-align="center" width="526" height="335"}
...
![](sampling_protocol.png){fig-align="center" width="374"}
...
categorical_regression_herbivors %>% 
  ...
  rename(Estimate = Value,
         "P Value" = `p value`) %>% 
  select(Coefficients, Estimate, `P Value`) %>% 
  mutate(" " = as.character(star_function(`P Value`))) %>% 
  mutate_at(vars(Estimate, `P Value`), ~round(., digits = 4)) %>%
